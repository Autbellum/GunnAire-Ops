...
| --- | --- | --- |
| iPad Simulator test suite | iPad (A16), iOS 26.4.1 and 26.5; unit result bundle `/tmp/GunnAireOps-qbo-workspace-unit.xcresult` and UI result bundle `/tmp/GunnAireOps-qbo-workspace-ui-full.xcresult`. | **364 passed (355 unit + 9 unique UI), 0 failed, 0 skipped**; includes CloudKit readiness/schema persistence, focused job-stage and QuickBooks workspace policies, field-payment Handoff and initial-closeout guards, recommended Billing-stage navigation, direct Schedule-to-invoice collection, dispatch-board date/conflict/ownership/editing rules, role navigation, catalog-state lifecycle, reconciliation, quantity/legacy invoice snapshots, and secure launch coverage. |
...
| iPad UI navigation suite | iPad (A16), iOS 26.5, `xcodebuild test-without-building -only-testing:'GunnAire OpsUITests'`; current result bundle `/tmp/GunnAireOps-qbo-workspace-ui-full.xcresult`. | **9 unique tests passed, 0 failed** (12 executions including launch variants); verifies secure unauthenticated launch, authenticated administrator iPad workspace, primary sidebar navigation, dispatch-board job editing, recommended Billing-stage documentation, direct Schedule-to-invoice collection, focused QuickBooks accounting workspaces, and launch performance. |
| Mac Catalyst compile/signing | Unsigned Release `xcodebuild` Mac Catalyst build rerun after the focused QuickBooks accounting workspace plus a signed Debug Catalyst build using Apple Development and the generated `Mac Catalyst Team Provisioning Profile: com.gunnaire.businesssuite`. | Passed; App Store distribution profile/archive remains a release gate. |
| iPad/iPhone Simulator Release compile | Generic unsigned Release build rerun after the focused QuickBooks accounting workspace and development-only CloudKit schema bootstrap. | Passed; the bootstrap is excluded with `#if DEBUG`. |
