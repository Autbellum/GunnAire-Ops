...
| --- | --- | --- |
| iPad Simulator test suite | iPad Pro 13-inch (M5), iOS 26.5; unit result bundle `/tmp/GunnAireOps-handoff-final-unit.xcresult` and UI result bundle `/tmp/GunnAireOps-handoff-final-ui.xcresult`. | **359 passed (352 unit + 7 UI), 0 failed, 0 skipped**; includes CloudKit readiness/schema persistence, field-payment Handoff and initial-closeout guards, direct Schedule-to-invoice collection, dispatch-board date/conflict/ownership/editing rules, role navigation, catalog-state lifecycle, reconciliation, quantity/legacy invoice snapshots, and secure launch coverage. |
...
| iPad UI navigation suite | iPad Pro 13-inch (M5), iOS 26.5, `xcodebuild test -only-testing:'GunnAire OpsUITests'`; current result bundle `/tmp/GunnAireOps-handoff-final-ui.xcresult`. | **7 tests passed, 0 failed** (10 runs including launch variants); verifies secure unauthenticated launch, authenticated administrator iPad workspace, primary sidebar navigation, dispatch-board job editing, direct Schedule-to-invoice collection, and launch performance. |
