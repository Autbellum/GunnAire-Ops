...
| Google Gmail/Calendar | Google sign-in, calendar create/update/delete only for app-managed events, Gmail delivery records, and account-scoped routing. Google Auth Platform is Internal and the current iOS client has the correct Apple Team ID, but it is bound to the superseded bundle ID `www.gunnaire.com.GunnAire-Ops`. | Project verified; OAuth client replacement required | Create an iOS OAuth client for `com.gunnaire.businesssuite`, update the app/backend public client ID, confirm scopes, and run production-account tests. |
...
| --- | --- | --- |
| iPad Simulator test suite | iPad Pro 13-inch (M5), iOS 26.5, `xcodebuild test`; current result bundle `/tmp/GunnAireOps-completion-audit-20260826.xcresult`. | **345 passed, 0 failed, 0 skipped**; includes CloudKit readiness/schema persistence, field-payment Handoff guards, sequential field-task prompts, catalog-state lifecycle, catalog reconciliation, and quantity/legacy invoice-snapshot coverage. |
