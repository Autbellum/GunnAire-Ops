...
| --- | --- | --- |
| iPad Simulator test suite | iPad (A16), iOS 26.4.1 and 26.5; unit result bundle `/tmp/GunnAireOps-staged-unit.xcresult` and UI result bundle `/tmp/GunnAireOps-staged-ui-full.xcresult`. | **362 passed (354 unit + 8 unique UI), 0 failed, 0 skipped**; includes CloudKit readiness/schema persistence, focused job-stage policy, field-payment Handoff and initial-closeout guards, recommended Billing-stage navigation, direct Schedule-to-invoice collection, dispatch-board date/conflict/ownership/editing rules, role navigation, catalog-state lifecycle, reconciliation, quantity/legacy invoice snapshots, and secure launch coverage. |
...
| iPad UI navigation suite | iPad (A16), iOS 26.5, `xcodebuild test-without-building -only-testing:'GunnAire OpsUITests'`; current result bundle `/tmp/GunnAireOps-staged-ui-full.xcresult`. | **8 unique tests passed, 0 failed** (11 executions including launch variants); verifies secure unauthenticated launch, authenticated administrator iPad workspace, primary sidebar navigation, dispatch-board job editing, recommended Billing-stage documentation, direct Schedule-to-invoice collection, and launch performance. |
