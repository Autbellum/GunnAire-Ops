...
| --- | --- | --- |
| iPad Simulator test suite | iPad Pro 13-inch (M5), iOS 26.5, combined `xcodebuild test`; current result bundle `/tmp/GunnAireOps-cloudkit-production-audit-20260826.xcresult`. | **350 passed (345 unit + 5 UI), 0 failed, 0 skipped**; includes CloudKit readiness/schema persistence, field-payment Handoff guards, role navigation, catalog-state lifecycle, reconciliation, quantity/legacy invoice snapshots, and secure launch coverage. |
...
| iPad UI navigation suite | iPad Pro 13-inch (M5), iOS 26.5, `xcodebuild test -only-testing:'GunnAire OpsUITests'`; current result bundle `/tmp/GunnAireOps-ui-audit-20260826.xcresult`. | **5 tests passed, 0 failed** (8 runs including launch variants); verifies secure unauthenticated launch, authenticated administrator iPad workspace, primary sidebar navigation, and launch performance. |
| Mac Catalyst compile/signing | Unsigned Release `xcodebuild` Mac Catalyst build plus a signed Debug Catalyst build using Apple Development and the generated `Mac Catalyst Team Provisioning Profile: com.gunnaire.businesssuite`. | Passed; App Store distribution profile/archive remains a release gate. |
| iPad/iPhone Simulator Release compile | Generic unsigned Release build rerun after adding the development-only CloudKit schema bootstrap. | Passed; the bootstrap is excluded with `#if DEBUG`. |
