...
Subscribe the entities the app reconciles: Customer, Item, Estimate, Invoice, Vendor, Bill, VendorCredit, Purchase, Payment, SalesReceipt, Deposit, PaymentMethod, and Attachable. Use Intuit's portal test-event function after both sandbox and production are configured. Until the verifier token exists, the endpoint fails closed and backend readiness reports QuickBooks Change Alerts as requiring attention.
