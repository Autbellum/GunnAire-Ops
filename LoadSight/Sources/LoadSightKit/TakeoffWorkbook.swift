...
public enum TakeoffWorkbook {
    ...
    public static func xlsx(_ project: ProjectDocument, drawings: DrawingArchive? = nil) throws -> Data {
        ...
        for (index, sheet) in sheets.enumerated() {
            let message = "Export snapshot - not for bid release. Blank quantities are unknown. Review issues: \(review.blockers.count)."
            add("xl/worksheets/sheet\(index + 1).xml", try worksheet(title: sheet.0 + " - " + project.name, note: message, headers: sheet.1, rows: sheet.2, widths: sheet.3, dateColumn: index == 1 ? 8 : (index == 2 ? 4 : nil)))
        }
    ...
    private static func cell(_ value: JSONValue, row: Int, col: Int, style: Int = 0) throws -> String {
        ...
        if let number = value.number { return "<c r=\"\(address)\" s=\"\(style == 5 ? 5 : 3)\"><v>\(number)</v></c>" }
    ...
    private static func worksheet(title: String, note: String, headers: [String], rows: [[JSONValue]], widths: [Double], dateColumn: Int?) throws -> String {
        ...
        body += "<row r=\"5\" ht=\"30\" customHeight=\"1\">" + (try headers.enumerated().map { try cell(.string($0.element), row: 5, col: $0.offset, style: 1) }.joined()) + "</row>"
        for (index, values) in rows.enumerated() {
            ...
            body += "<row r=\"\(index + 6)\" ht=\"\(height)\" customHeight=\"1\">" + (try values.enumerated().map { try cell($0.element, row: index + 6, col: $0.offset, style: $0.offset == dateColumn ? 5 : 0) }.joined()) + "</row>"
        }
    ...
    <styleSheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main"><fonts count="3"><font><sz val="10"/><name val="Arial"/></font><font><b/><color rgb="FFFFFFFF"/><sz val="10"/><name val="Arial"/></font><font><b/><sz val="14"/><name val="Arial"/></font></fonts><fills count="3"><fill><patternFill patternType="none"/></fill><fill><patternFill patternType="gray125"/></fill><fill><patternFill patternType="solid"><fgColor rgb="FF243746"/></patternFill></fill></fills><borders count="1"><border/></borders><cellStyleXfs count="1"><xf/></cellStyleXfs><cellXfs count="6"><xf fontId="0" fillId="0" borderId="0" xfId="0" applyAlignment="1"><alignment vertical="top" wrapText="1"/></xf><xf fontId="1" fillId="2" borderId="0" xfId="0" applyAlignment="1"><alignment horizontal="center" vertical="center" wrapText="1"/></xf><xf fontId="2" fillId="0" borderId="0" xfId="0"/><xf fontId="0" fillId="0" borderId="0" numFmtId="0" xfId="0" applyNumberFormat="1"><alignment horizontal="right" vertical="top"/></xf><xf fontId="0" fillId="0" borderId="0" xfId="0"/><xf fontId="0" fillId="0" borderId="0" numFmtId="14" xfId="0" applyNumberFormat="1"><alignment horizontal="right" vertical="top"/></xf></cellXfs><cellStyles count="1"><cellStyle name="Normal" xfId="0" builtinId="0"/></cellStyles></styleSheet>
    """
}
