...
def run_suite(
    ...
    *,
    ...
    run_id = dt.datetime.now().strftime("%Y%m%d-%H%M%S") + "-" + suite.name
    run_dir = report_root.expanduser() / run_id
    run_dir.mkdir(parents=True, exist_ok=False)
    results: list[dict[str, Any]] = []
    for index, command in enumerate(suite.commands, start=1):
        result = run_command(command, cwd=resolved_repo, timeout_seconds=suite.timeout_seconds, environment=environment)
