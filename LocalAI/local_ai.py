    ...
    if host.lower() == "localhost":
        ...
        try:
            ...
            try:
                ...
                raise PolicyError(f"Cannot resolve local endpoint host {host!r}: {exc}") from exc
...
def doctor(config: Config, policy: Policy, client: OllamaClient) -> dict[str, Any]:
    ...
    return {
        "status": "ok" if required_present else "missing-required-models",
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        if args.command == "doctor":
            _emit(doctor(config, policy, client), args.output)
