...
- QBO authorization-code exchange, refresh, and revoke through the backend. The QBO client secret is server-only and is no longer included in the iOS app's Info.plist. QuickBooks Management is separated into Overview, Sales, Expenses, and Payments workspaces so accounting users can focus on one operational lane while retaining a persistent read-only Sync action.
