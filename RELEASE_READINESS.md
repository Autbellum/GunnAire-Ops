...
The iOS target builds for the configured iPhone and iPad simulators and launches successfully. The current iPad verification passed **390 unique tests** (**371 unit + 19 unique UI**) on an iPad (A16) simulator running iOS 26.5 on 2026-08-26 with **0 failures and 0 skipped tests**. The result recorded 393 device executions because one launch test runs in four appearance/orientation combinations. Generic arm64 iOS device and universal arm64/x86_64 Mac Catalyst **Release** builds also compile successfully, and both binaries are clean of the Debug-only test identities, fixtures, and CloudKit schema-bootstrap launch flags. Release builds use the deployed HTTPS Render backend and explicitly clear the legacy shared API token, preventing a developer LAN endpoint or static administrator-equivalent token from being archived accidentally. The Render root entrypoint delegates to the canonical tested backend; the current deployment adds an Admin-only readiness contract and manifest-backed backup/restore-drill tooling while retaining encrypted, server-held QBO refresh tokens. Production deployment of `serviceVersion: 2026.08.26.3` is verified after the corresponding commit is live. The corrective-work and invoice-sync CloudKit field migration is deployed and verified in Production. The project is still **not ready to upload to TestFlight, Mac App Store, or submit to App Review** until the physical-device, OAuth, privacy, App Store Connect, and Mac Catalyst provisioning requirements below are complete. See `COMPLETION_EVIDENCE_MATRIX.md` for the product-by-product evidence and gate owners.
...
python3 -m unittest Backend.test_deployment_entrypoint Backend.test_qbo_token_storage \
  Backend.test_backend_readiness Backend.test_backup_backend -v
```
