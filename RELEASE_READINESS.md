...
- A device-level CloudKit readiness check is shown in administrator settings before staff rely on cross-device continuity. The 21-model SwiftData record types, 36-field `CD_ServiceCall` schema, and 16-field `CD_Invoice` schema are deployed in Production. The optional `CD_Customer.storedPaymentMethodsJSON`, `CD_ServiceCall.maintenanceAgreementID`, and `CD_ServiceCall.maintenanceAgreementDueDate` fields are implemented and locally verified but must still be initialized and deployed in CloudKit Production before TestFlight. Private CloudKit sync supports the approved company iCloud account across its devices; it is not a replacement for the shared-company backend when staff use individual iCloud accounts.
