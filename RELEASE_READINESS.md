...
The iOS target builds for the configured iPhone and iPad simulators and launches successfully. The current iPad verification passed **419 unique tests** (**393 unit + 26 unique UI**) on an iPad Pro 13-inch (M5) simulator running iOS 26.5 on 2026-08-27 with **0 failures and 0 skipped tests**. The results recorded 422 device executions because one launch test runs in four appearance/orientation combinations. Generic arm64 iOS device and universal arm64/x86_64 Mac Catalyst **Release** builds also compile successfully after the confirmed-send Gmail workflow changes, and both binaries are clean of Debug-only test identities, fixtures, and CloudKit bootstrap flags. Release builds use the deployed HTTPS Render backend and clear the legacy shared API token. The Render entrypoint delegates to the canonical tested backend; the current deployment adds secure QBO CloudEvents change awareness to its Admin-only readiness and backup controls while retaining encrypted, server-held refresh tokens. Production deployment of `serviceVersion: 2026.08.26.4` is verified after the corresponding commit is live. On 2026-08-27 Apple deployed and CloudKit Console verified the remaining optional Customer/ServiceCall/Estimate/Technician/Item schema fields in Production. The project is still **not ready to upload to TestFlight, Mac App Store, or submit to App Review** until the physical-device, OAuth, privacy, App Store Connect, and Mac Catalyst provisioning requirements below are complete. See `COMPLETION_EVIDENCE_MATRIX.md` for the product-by-product evidence and gate owners.
...
- A device-level CloudKit readiness check is shown in administrator settings before staff rely on cross-device continuity. All 21 SwiftData record types and the current optional business fields are deployed in Production. CloudKit Console verified Customer 14 fields, Estimate 23, Item 20, ServiceCall 38, Technician 12, and Invoice 16 on 2026-08-27, including stored-payment metadata, maintenance lineage, estimate approval and scheduled-work lineage, technician QBO mappings, and pricebook review audit fields. Matching Development and Production schema exports were retained. Private CloudKit sync supports the approved company iCloud account across its devices; it is not a replacement for the shared-company backend when staff use individual iCloud accounts.
...
1. Verify two-device corrective-job, stored-payment-reference, maintenance-obligation, estimate-approval-to-scheduled-work lineage, technician QBO worker-mapping, and pricebook-review round trips on signed company hardware, including offline edits and recovery. The complete current schema was deployed and verified in Production on 2026-08-27; this gate is now physical-device acceptance rather than schema deployment.
