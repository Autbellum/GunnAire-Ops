...
The iOS target builds for the configured iPhone and iPad simulators and launches successfully. The current iPad verification passed **388 unique tests** (**370 unit + 18 unique UI**) on an iPad (A16) simulator running iOS 26.5 on 2026-08-26 with **0 failures and 0 skipped tests**. The result recorded 391 device executions because one launch test runs in four appearance/orientation combinations. Generic arm64 iOS device and universal arm64/x86_64 Mac Catalyst **Release** builds also compile successfully, and both binaries are clean of the Debug-only test identities, fixtures, and CloudKit schema-bootstrap launch flags. Release builds use the deployed HTTPS Render backend and explicitly clear the legacy shared API token, preventing a developer LAN endpoint or static administrator-equivalent token from being archived accidentally. The Render root entrypoint now delegates to the canonical tested backend; production returned HTTP 200 with `serviceVersion: 2026.08.26.2` after commit `5a4d58e`, proving the encrypted, server-retained QBO refresh-token implementation is live instead of the older duplicate service copy. The corrective-work and invoice-sync CloudKit field migration is deployed and verified in Production. The project is still **not ready to upload to TestFlight, Mac App Store, or submit to App Review** until the physical-device, OAuth, privacy, App Store Connect, and Mac Catalyst provisioning requirements below are complete. See `COMPLETION_EVIDENCE_MATRIX.md` for the product-by-product evidence and gate owners.
...
- A small Python/SQLite backend for approved users and roles, shared documents, communications, payment records, and QBO confidential OAuth operations. Render's root launcher imports this canonical implementation rather than maintaining a second copy; deployment tests protect the dependency and no-refresh-token client contract. See `Backend/README.md`.
...
- A device-level CloudKit readiness check is shown in administrator settings before staff rely on cross-device continuity. The 21-model SwiftData record-type schema and its current field additions are deployed in Production. CloudKit Console verifies 36 `CD_ServiceCall` fields, including the three optional corrective-work lineage/reason fields, and 16 `CD_Invoice` fields including `CD_quickBooksSyncStatus`. Private CloudKit sync supports the approved company iCloud account across its devices; it is not a replacement for the shared-company backend when staff use individual iCloud accounts.
