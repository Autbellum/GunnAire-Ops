...
The iOS target builds for the configured iPhone and iPad simulators and launches successfully. The current iPad verification passed **424 unique tests** (**398 unit + 26 unique UI**) on iPad Pro 13-inch (M5) simulators running iOS 26.2 and 26.5 on 2026-08-27 with **0 failures and 0 skipped tests**. The results recorded 427 device executions because one launch test runs in four appearance/orientation combinations. Generic arm64 iOS device and universal arm64/x86_64 Mac Catalyst **Release** builds also compile successfully after the reusable service-location changes, and both binaries are clean of Debug-only test identities, fixtures, and CloudKit bootstrap flags. Release builds use the deployed HTTPS Render backend and clear the legacy shared API token. The Render entrypoint delegates to the canonical tested backend; the current deployment adds secure QBO CloudEvents change awareness to its Admin-only readiness and backup controls while retaining encrypted, server-held refresh tokens. Production deployment of `serviceVersion: 2026.08.26.4` is verified after the corresponding commit is live. On 2026-08-27 Apple deployed and CloudKit Console verified the prior optional Customer/ServiceCall/Estimate/Technician/Item schema fields in Production. The version-seven service-location schema is verified in Development and awaits the final authenticated Production deployment. The project is still **not ready to upload to TestFlight, Mac App Store, or submit to App Review** until the physical-device, OAuth, privacy, App Store Connect, CloudKit Production, and Mac Catalyst provisioning requirements below are complete. See `COMPLETION_EVIDENCE_MATRIX.md` for the product-by-product evidence and gate owners.
...
- A device-level CloudKit readiness check is shown in administrator settings before staff rely on cross-device continuity. Production contains the prior 21-model schema. The signed version-seven bootstrap initialized all 22 current models in Development, including the new 18-field `CD_CustomerServiceLocation` type, `CD_CustomerEquipment.serviceLocationID`, and the two ServiceCall property fields. Development now reports CustomerEquipment 13, CustomerServiceLocation 18, and ServiceCall 40 fields. Production deployment and a matching Production schema export remain required. Private CloudKit sync supports the approved company iCloud account across its devices; it is not a replacement for the shared-company backend when staff use individual iCloud accounts.
...
1. Deploy and verify the version-seven schema in Production, retain matching Development and Production exports, then verify two-device service-location/equipment/job, corrective-job, stored-payment-reference, maintenance-obligation, estimate-approval-to-scheduled-work lineage, technician QBO worker-mapping, and pricebook-review round trips on signed company hardware, including offline edits and recovery.
