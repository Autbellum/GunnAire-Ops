...
2. Replace the Google iOS OAuth client: the inspected client uses the old bundle ID `www.gunnaire.com.GunnAire-Ops`, but the current target is `com.gunnaire.businesssuite`. Create a new iOS client for the current bundle and Apple Team ID, update the app and backend public client ID together, then verify sign-in, Gmail send, Calendar create/update/delete, and backend role denial with production accounts. Intuit Development and Production already contain the exact `https://gunnaire.com/wp-json/ga/v1/qbo/oauth/callback` redirect; still verify token refresh and QBO company selection end to end.
