...
final class PersistenceTests: XCTestCase {
    ...
    func testEveryFieldSurvivesRoundTrip() throws {
        ...
        project.supplyAirDeltaTF = 22
        project.sizingLimits.coolingMinimumFraction = 1.02
        project.staticPressureBudget.filter = 0.14
        project.equipment.modelNumber = "XR16-036"
        project.zones[0].surfaces[0].coolingEquivalentDeltaTF = 41
        project.ductRuns[1].fittings.append(Fitting(name: "Extra elbow", equivalentLengthFeet: 12, count: 3))
        ...
        XCTAssertEqual(restored.ductRuns[1].fittings.last?.count, 3)
        XCTAssertEqual(restored.equipment.modelNumber, "XR16-036")
    }
    ...
    func testZoneIdentityAndDuctLinksSurvive() throws {
        ...
        let restored = try roundTrip(project)
        let servedOriginal = project.ductRuns.compactMap(\.servingZoneID)
        let servedRestored = restored.ductRuns.compactMap(\.servingZoneID)
    ...
    func testReopenedProjectReproducesTheSameDesign() throws {
        ...
        XCTAssertEqual(reopened.selection?.checks.count, engine.selection?.checks.count)
    }
